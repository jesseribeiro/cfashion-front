<template>
  <v-container
    fluid
    grid-list-md>
    <span class="title">Acordos Efetuados e Acordos em atraso</span>
    <v-form
      ref="form2"
          ...
          <v-card class="elevation-0">
            <v-card-text>
              <v-layout
                ...
                <v-flex md4>
                  <v-autocomplete
                    v-model="filtros.tpSituacaoConta"
                    :items="situacaoConta"
                    :error-messages="errors.collect('Situação Conta')"
                    label="Situação Conta"
                    item-value="id"
                    item-text="descricao"
                  />
                </v-flex>
                <v-flex md4>
                  <v-autocomplete
                    v-model="filtros.tpSituacaoAcordo"
                    :items="situacaoAcordo"
                    label="Situação Acordo"
                    item-value="id"
                    item-text="descricao"
                  />
                </v-flex>
...
export default {
  ...
  data () {
    return {
      filtros: {
        ...
        fimAtraso: null,
        tpPeriodo: 'SETE_DIAS',
        tpSituacaoConta: 'TODAS',
        tpSituacaoAcordo: 'EM_ABERTO',
  ...
  beforeMount () {
    ...
    TiposBusiness.getAllStatusCarne()
      .then(response => {
        this.situacaoAcordo = response.data
      })
    TiposBusiness.getAllPeriodoRelatorio()
      .then(response => {
        this.periodos = response.data
      })
    TiposBusiness.getAllSituacaoConta()
    .then(response => {
      this.situacaoConta = response.data
    })
  ...
  methods: {
    validateBeforeSubmit () {
      if (this.filtros.dataInicio > this.filtros.dataFim) {
        this.$root.showAlerta(`A data inicial não pode ser mais que a data final`)
      }
      this.$validator.validate().then(result => {
        if (result) {
          ...
          RelatorioBusiness.geraAcordosEfetuados(this.filtros)
            .then(response => {
