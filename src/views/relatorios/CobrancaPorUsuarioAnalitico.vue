<template>
  <div>
    <vScope
      ...
      :file-pdf="filePDF"
      :filtros="filtros"
      titulo="Cobrança Por Usuário (Analítico)"
...
export default {
  ...
  beforeMount() {
    this.loading = true
    var promiseTiposRelatorio = TiposBusiness.getAllTiposRelatorio().then((response) => {
      this.tiposRel = response.data;
    });

    var promiseTiposOcorrencia = TiposBusiness.getAllTiposOcorrencia()
      .then((response) => {
    ...
    var promiseNegociadores = null
    if(!this.$root.isNegoc()){
      this.listaUsersPratico.push({id: TODAS_ID, nome: "Todos"})
      promiseNegociadores = UsuarioBusiness.listAllNegociador()
        .then(response => {
    ...
    Promise.all([
      promiseTiposRelatorio,
      promiseTiposOcorrencia,
      promiseNegociadores
    ]).finally(() => {
      this.loading = false
    })
    ...
    validateBeforeSubmit() {
      this.$validator.validate().then((result) => {
        if (result && !!this.filtros.lojaId) {
          ...
          RelatorioBusiness.geraRelatorioCobrancaPorUsuarioAnalitico(
            this.filtros
          )
            .then((response) => {
