<template>
  <v-container
    fluid
    grid-list-md>
    <span class="title">Consultas no SPC (Resumo)</span>
    <v-form
      ref="form2"
              ...
              <v-layout
                ...
                <v-flex md4>
                  <v-checkbox
                    ...
                    label="Totalizar por grupo"
                  />
                </v-flex>
                <v-flex md4>
                  <v-checkbox
                    v-model="filtros.listarContas"
                    label="Incluir consultas sem rede"
                  />
                </v-flex>
              </v-layout>
            </v-card-text>
    ...
    validateBeforeSubmit () {
      this.$validator.validate().then(result => {
        if (result) {
          ...
          RelatorioBusiness.geraConsultaSpcAnalitico(this.filtros)
            .then(response => {
