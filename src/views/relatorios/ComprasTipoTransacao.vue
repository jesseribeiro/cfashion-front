<template>
  <v-container
    fluid
    grid-list-md>
    <span class="title">Compras por tipo de transação</span>
    <v-form
      ref="form2"
...
export default {
  ...
  data () {
    return {
      ...
      loading: false,
      tiposStatus: [
        { id: 'WEB', descricao: 'WEB' },
        { id: 'TEF', descricao: 'TEF' },
        { id: 'POS', descricao: 'POS' }
      ],
      grupos: [
    ...
    validateBeforeSubmit () {
      this.$validator.validate().then(result => {
        if (result) {
          ...
          RelatorioBusiness.geraComprasTipoTransacao(this.filtros)
            .then(response => {
