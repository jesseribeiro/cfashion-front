<template>
  <v-container 
    fluid 
    grid-list-md>
    <span class="title">Movimento de Compras</span>
    <v-form 
      ref="form2" 
...
export default {
  ...
  beforeMount() {
    ...
    TiposBusiness.tiposLancamentoRepasse().then((response) => {
    ...
    validateBeforeSubmit() {
      this.$validator.validate().then((result) => {
        if (result) {
          ...
          RelatorioBusiness.geraMovimentoCompras(this.filtros)
            .then((response) => {
