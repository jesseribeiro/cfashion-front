<template>
  <v-container
    fluid
    grid-list-md>
    <span class="title">Relatório Repasse de Lançamentos Analíticos</span>
    <v-form
      ref="form2"
              ...
              <v-layout
                ...
                <v-flex md4>
                  <v-autocomplete
                    ...
                    :items="tiposLanc"
                    :error-messages="errors.collect('Tipo de Lançamento')"
                    data-vv-name="Tipo de Lançamento"
                    label="Tipo"
                    item-value="id"
                    item-text="descricao"
                  />
    ...
    validateBeforeSubmit () {
      this.$validator.validate().then(result => {
        if (result) {
          ...
          RelatorioBusiness.geraRepasseLancamentos(this.filtros)
            .then(response => {
