...
export default {
  ...
  data() {
    return {
      ...
      listaFaixaAtraso: [
        { id: 2, descricao: 'Detalhada' },
        { id: 1, descricao: 'BACEN' }
      ],
      listaPor: [
        { id: 1, descricao: 'Saldo devedor' },
        { id: 2, descricao: 'Principal(compras)' },
      ],
    };
  },
  methods: {
    validateBeforeSubmit() {
      this.$validator.validate().then((result) => {
        if (result) {
          ...
          RelatorioBusiness.gerarGerenciaisResumoInadimplencia(this.filtros)
            .then((response) => {
