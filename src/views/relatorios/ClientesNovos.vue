<template>
  <v-container
    fluid
    grid-list-md>
    <span class="title">Clientes Novos</span>
    <v-form
      ref="form2"
          ...
          <v-card class="elevation-0">
            <v-card-text>
              <v-layout
                ...
                <v-flex md3>
                  <v-checkbox
                    v-model="filtros.comprasCanceladas"
                    label="Incluir compras canceladas"
                  />
                </v-flex>
              </v-layout>
            </v-card-text>
    ...
    validateBeforeSubmit () {
      this.$validator.validate().then(result => {
        if (result) {
          ...
          RelatorioBusiness.geraClientesNovos(this.filtros)
            .then(response => {
